    ...
    const speakPrayer = async () => {
        if (language !== 'Choose Language') {
            const prayerTexts = [
                ...
                { title: 'Psalms 23', text: prayers[language]?.prayer },
                { title: 'Psalms 91', text: prayers[language]?.prayer1 }
            ];
            setIsPlaying(true);
            ...
            try {
                for (const prayer of prayerTexts) {
                    ...
                    await playGoogleTTS(prayer.text, 0); // 0.5s pause between prayers
                }
            } catch (error) {
